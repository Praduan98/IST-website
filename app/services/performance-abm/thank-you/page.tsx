...
export default function PerformanceABMThankYouPage() {
...
function HeroSection() {
  ...
  const handleDownload = useCallback(async () => {
    try {
      const res = await fetch("/assets/InsightsTap-Performance-Marketing-ABM-Brochure.pdf")
      if (res.ok) {
        const blob = await res.blob()
        const url = URL.createObjectURL(blob)
        const a = document.createElement("a")
        a.href = url; a.download = "InsightsTap-Performance-Marketing-ABM-Brochure.pdf"
  ...
  return (
    <section className="relative flex min-h-[85vh] flex-col items-center justify-center overflow-hidden bg-white" style={{ paddingTop: "15vh", paddingBottom: "15vh" }}>
      ...
      <div className="relative z-10 mx-auto w-[min(92vw,720px)] px-6 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3, ease: [0.25, 0.46, 0.45, 0.94] }} className="mb-5 text-[2.5rem] font-semibold leading-[1.1] tracking-tight text-[#0F172A] sm:text-5xl lg:text-[3.5rem]">
          Your Playbook is <span className="gradient-text">Ready</span>.
        </motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.6 }} className="mx-auto mb-10 max-w-[540px] text-[1.125rem] leading-relaxed text-[#64748B]">
          Click below to download the Performance Marketing & ABM Playbook, or book a discovery call to discuss your campaign strategy.
        </motion.p>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.8 }} className="flex flex-col items-center justify-center gap-4 sm:flex-row">
          <button onClick={handleDownload} className="shimmer relative h-12 rounded-lg bg-[#0dcfcf] px-8 text-[1.2rem] font-medium text-white shadow-md shadow-[#0dcfcf]/15 transition-all hover:-translate-y-0.5 hover:bg-[#0a9a9a] hover:shadow-lg hover:shadow-[#0dcfcf]/25 inline-flex items-center justify-center gap-2 cursor-pointer">
