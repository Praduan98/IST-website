...
export default function CRMOptinPage() {
    ...
    <section className="relative flex min-h-[80vh] flex-col items-center justify-center overflow-hidden bg-white pt-28 pb-16">
      ...
      <div className="relative z-10 mx-auto w-[min(92vw,1600px)] px-6 text-center">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.6 }} className="mx-auto mb-14 max-w-[660px] text-base leading-relaxed text-[#64748B] sm:text-lg">
          Discover how we automate CRM enrichment, clean dirty data pipelines, and connect every touchpoint — so your sales team always has the full picture.
        </motion.p>
...
function FormSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fields = [
      ...
      { name: "dr_code", value: "DR021" }, { name: "form_code", value: "FPG036" },
      { name: "ist_lead_source", value: "CRM Playbook Download" },
    ...
    try {
      if (HS_PORTAL_ID && HS_FORM_GUID) {
        const res = await fetch(`https://api.hsforms.com/submissions/v3/integration/submit/${HS_PORTAL_ID}/${HS_FORM_GUID}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fields, context: { pageUri, pageName: "CRM Playbook Download" } }) })
        if (!res.ok) { setStatus("error"); return }
      }
      router.push("/services/crm-automation/thank-you")
    ...
    <section ref={sectionRef} className="relative bg-white px-6 overflow-hidden" style={{ paddingTop: "15vh", paddingBottom: "15vh" }}>
      ...
      <div className="relative z-10 mx-auto w-[min(92vw,680px)]">
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5 }} className="mb-10 text-center">
          ...
          <p className="mx-auto max-w-[480px] text-base leading-relaxed text-[#64748B]">Fill in your details to unlock the CRM Enrichment & Automation Playbook on the next page.</p>
        </motion.div>
