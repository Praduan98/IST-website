...
function HeroSection() {
  ...
  const handleDownload = useCallback(async () => {
    try {
      // Try gated token flow first
      const tokenRes = await fetch("/api/playbook-token", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: "verified-user", formCode: "FPG034" }),
      })
      ...
      if (tokenRes.ok) {
        ...
        const pdfRes = await fetch(
          `/api/fetch-playbook?token=${encodeURIComponent(token)}`
        )
        if (pdfRes.ok) {
      ...
      if (!blob) {
        const fallback = await fetch(
          "/assets/InsightsTap-AI-GTM-Strategy-Playbook.pdf"
        )
        if (fallback.ok) {
      ...
      if (blob) {
        ...
        a.href = url
        a.download = "InsightsTap-AI-GTM-Strategy-Playbook.pdf"
        document.body.appendChild(a)
        a.click()
        document.body.removeChild(a)
        setTimeout(() => URL.revokeObjectURL(url), 1000)

        // Show "Download Done" toast
        setShowToast(true)
    ...
    >
      ...
      <AnimatePresence>
        {showToast && (
          <motion.div
            ...
            className="fixed right-6 top-24 z-50 flex items-center gap-3 rounded-xl border border-[#0dcfcf]/30 bg-white px-5 py-3.5 shadow-xl shadow-black/10"
          >
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#0dcfcf]/12">
              <CheckCircle className="h-4 w-4 text-[#0dcfcf]" />
            </div>
            <div>
      ...
      <div className="relative z-10 mx-auto w-[min(92vw,720px)] px-6 text-center">
        ...
        <motion.h1
          ...
          className="mb-5 whitespace-nowrap text-3xl font-semibold leading-[1.05] tracking-tight text-[#0F172A] sm:text-4xl md:text-5xl lg:text-[3.5rem]"
        >
          Your playbook is{" "}
...
// ─── Floating dots (atmospheric) ───────────────────────────────────────────────
const FLOAT_DOTS = [
  { x: 5, y: 10, dur: 7, delay: 0 },
  { x: 20, y: 45, dur: 9, delay: 1.2 },
  { x: 35, y: 78, dur: 6, delay: 2.4 },
  { x: 50, y: 22, dur: 8, delay: 0.6 },
  { x: 65, y: 60, dur: 7, delay: 1.8 },
  { x: 78, y: 35, dur: 9, delay: 3 },
  { x: 90, y: 80, dur: 6, delay: 0.3 },
  { x: 12, y: 88, dur: 8, delay: 1.5 },
]
