...
// ─── Page ──────────────────────────────────────────────────────────────────────
export default function CustomAIOptinPage() {
...
function FormSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fields = [
      ...
      { name: "dr_code", value: "DR019" },
      { name: "form_code", value: "FPG035" },
      { name: "ist_lead_source", value: "Custom AI Brochure Download" },
    ...
    try {
      if (HS_PORTAL_ID && HS_FORM_GUID) {
        const res = await fetch(
          `https://api.hsforms.com/submissions/v3/integration/submit/${HS_PORTAL_ID}/${HS_FORM_GUID}`,
          {
            ...
            body: JSON.stringify({
              fields,
              context: { pageUri, pageName: "Custom AI Brochure Download" },
            }),
          }
        )
        if (!res.ok) {
      ...
      router.push("/services/custom-ai-development/thank-you")
    } catch {
      setStatus("error")
    }
  }
